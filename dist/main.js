...
class GridStackManager {
        ...
        this.gridStack = GridStack.init({
            ...
            dragInOptions: {
                ...
                helper: function (event) {
                    const helper = event.currentTarget.cloneNode(true);
    ...
    setupWidgetEvents(widgetElement) {
        ...
        // Edit button click
        const editBtn = widgetElement.querySelector('.widget-edit-btn');
        if (editBtn) {
    ...
    setupEventListeners() {
        ...
        gridSelector.addEventListener('change', (e) => {
            this.changeGridType(e.target.value);
        });
        // Reset grid button
    ...
    updateWidgetLink(cellId, link) {
        const widget = document.querySelector(`[data-gs-id="${cellId}"]`);
        if (widget) {
            ...
            const gridWidget = this.gridStack.getGridItems().find((item) => item.gridstackNode?.id === cellId);
            if (gridWidget) {
    ...
    openLinkModal(cellId) {
        const widget = document.querySelector(`[data-gs-id="${cellId}"]`);
        if (!widget)
            return;
        const linkElement = widget.querySelector('.widget-link');
        const modal = document.getElementById('link-modal');
    ...
    handleWidgetEdit(id) {
        ...
        const widget = this.gridStack.engine.nodes.find((n) => n.id === id);
        if (widget) {
